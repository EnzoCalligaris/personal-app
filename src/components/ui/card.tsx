...
function Card({
  ...
  size?: "default" | "sm"
  /** Aplica hover/press quando o card inteiro é clicável. */
  interactive?: boolean
}) {
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl bg-card py-(--card-spacing) text-sm text-card-foreground shadow-soft ring-1 ring-border",
        ...
        interactive &&
          "cursor-pointer transition-all duration-200 hover:-translate-y-0.5 hover:shadow-raised hover:ring-foreground/15 active:translate-y-0 active:scale-[0.995]",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center gap-2 rounded-b-2xl border-t bg-muted/40 p-(--card-spacing)",
        className
      )}
      {...props}
    />
