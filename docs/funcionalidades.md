...
Como cada parte da plataforma funciona e por quê. Para a visão geral, veja o
[README](../README.md).
...
- **Sessão**: cookies geridos pelo `@supabase/ssr`, marcados `HttpOnly`, `SameSite=lax` e
  `Secure` em produção (`src/lib/supabase/cookies.ts`) - o cookie carrega o refresh token, então
  fica fora do alcance de JavaScript. A renovação acontece onde o cliente de servidor é criado:
  no proxy, nas navegações de página, e no próprio handler nas chamadas de API.
- **Role**: gravada em `app_metadata.role` no cadastro (`PERSONAL`/`ALUNO`). O proxy lê a role do
  token para redirecionar páginas, sem ida ao banco; as rotas de API leem **do banco**
  (`src/lib/auth/session.ts`), de modo que uma role alterada vale na hora, sem depender de o token
  ser renovado.
...
e observações. Em que dia da semana cada ficha cai é decidido na [programação](#programação-de-treinos).
...
| `POST /api/aluno/treinos/[id]/execucoes` | Fecha a sessão de treino (ver [Execução do treino](#execução-do-treino)) |
| `GET /api/aluno/historico` | Histórico de execuções, com o que foi feito em cada uma |
| `GET /api/aluno/agenda` | Agendamentos próximos e anteriores (ver [Agendamento pelo aluno](#agendamento-pelo-aluno)) |
...
| `GET /api/aluno/feedbacks` | Comentários do Personal (ver [Feedback do Personal](#feedback-do-personal)) |
